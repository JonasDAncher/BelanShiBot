...
async def reminder(interaction: discord.Interaction, players: list):
  ...
  print((datetime.datetime.now().timestamp()-interaction.created_at.timestamp()).seconds)
  print(len(players[0]) > 0)
  print(len(players[1]) > 0)
  print(len(players[2]) > 2)
  if isGroupFull(players):
    if (datetime.datetime.now(datetime.timezone.utc).timestamp()-interaction.created_at.timestamp()).seconds > 5:
